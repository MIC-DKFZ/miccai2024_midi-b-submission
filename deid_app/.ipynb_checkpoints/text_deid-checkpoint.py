...
from robust_deid.deid.utils import remove, replace_tag_type, replace_informative
...
class TextDeid(object):
    ...
    def __get_deid_text(
        ...
        age_pattern = r'((?<!\d+)([1-7]\d?)(?!\d+))|((?<!\d+)(8[0-8]?)(?!\d+))'
    ...
    def __get_super_relaxed_predictions(tokens, predictions):
        ...
        for index, (token, prediction) in enumerate(zip(tokens, relaxed_predictions)):
            ...
            if prediction == 'O' and prev_type is not None:
                # [a-zA-Z0-9]
                if re.search(r'^(\W|_)+$', token['text'], flags=re.IGNORECASE | re.DOTALL):
